      ...
      if (allUpdatedEpisodes.length > 0 || allSeasonCast.length > 0) {
        ...
        if (snapshotUser) {
          ...
          if (allSeasonCast.length > 0) {
            upsertSeasonCastsInDb(snapshotUser.id, snapshotTitle.id, allSeasonCast).catch((e) =>
              console.error('Season cast backfill DB write failed:', e)
            )
          }
          if (allEpisodeCrew.length > 0) {
            upsertEpisodeCrewsInDb(snapshotUser.id, snapshotTitle.id, allEpisodeCrew).catch((e) =>
