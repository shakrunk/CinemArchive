...
Restored from the audit removed during repository cleanup (`d9c282f`).
All actionable consistency findings are complete. Sections 02–06 shipped in
`f331753`; section 01 shipped in `ae5cd05` through the shared `Eyebrow`
component and its call-site migration. The stale KP-058 backlog entry is now
closed. Source paths below describe the original pre-monorepo tree; the web
app now lives in `apps/web/`.

The original intentionally distinct brand colors, poster art treatments, and
compact Ledger empty states remain intentional. Performance follow-through is
tracked in [the web performance audit](web-perf-audit.md).
...
**Severity:** High
**Status:** Fixed — `ae5cd05` introduced `Eyebrow` with shared size/tone tokens and migrated the repeated labels. Verified in `apps/web/src/components/ui/typography.tsx` and the Library, Discover, title drawer, sharing, and Ledger call sites.
