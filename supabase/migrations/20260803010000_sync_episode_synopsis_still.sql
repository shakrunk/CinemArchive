-- Republish the Android sync payload with episodes.synopsis/still_url (issue: title-detail
-- season selector/episode thumbnails on Android). Those two columns have existed on `episodes`
-- since the episode-tracking domain shipped and are written by the web app's TitleDetailDrawer
-- backfill (`upsertEpisodeMetadataInDb`), but `sync_library_changes`'s `episode` arm never
-- selected them, so Android silently dropped data that already exists for most web-touched
-- titles. Unchanged from 20260803000000_outing_seat_details.sql apart from the episode arm.
