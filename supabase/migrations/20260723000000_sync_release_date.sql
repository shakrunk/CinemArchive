-- Adds releaseDate to sync_library_changes()'s 'title' payload so the Android
-- client can distinguish an unreleased watchlist title from an available one
-- (parity with the web app's src/store/upNext.ts computeUpcomingTitles()).
-- titles.release_date has existed since 20260626000000_release_date.sql; it
-- was simply never included in the sync RPC's payload until now.
