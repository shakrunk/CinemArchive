...
export function mapDbOutingToLocal(row: any): CinemaOuting {
  return {
    ...
    auditorium: row.auditorium || undefined,
...
function mapDbTitleToLocal(row: any): Title {
  ...
  return {
    ...
    awardsCount: row.awards_count ?? undefined,
    bechdelOutcome: row.bechdel_outcome || undefined,
    bechdelScore: row.bechdel_score || undefined,
    studios: row.studios || [],
...
export async function insertTitleToDb(userId: string, title: Title): Promise<void> {
  ...
  const { error: titleError } = await supabase.from('titles').insert({
    ...
    rt_url: title.rtUrl ?? null,
    awards_count: title.awardsCount ?? null,
...
export async function insertOutingToDb(userId: string, outing: CinemaOuting): Promise<void> {
  ...
  const { error } = await supabase.from('cinema_outings').insert({
    ...
    auditorium: outing.auditorium ?? null,
    seat_row: outing.seatRow ?? null,
    seats: outing.seats ?? [],
