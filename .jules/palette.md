...
## 2024-11-20 - [Clear Search in Discover Empty States]
**Learning:** Empty states without actionable buttons rely on informative text, which increases cognitive load for users trying to recover from an empty result. Adding an explicit CTA to clear a search significantly improves user flow and interaction smoothness.
**Action:** Always provide direct call-to-action buttons (e.g., 'Clear search', 'Log viewing') in empty states instead of relying purely on informative text.
...
\n## 2026-07-06 - Ticket Cancellation Confirmation ARIA Labels\n**Learning:** The 'Yes' and 'No' buttons used for confirming ticket cancellations lacked unique `aria-label` attributes, creating ambiguity for screen reader users when multiple confirmation prompts might exist or simply lacking clear context (violating WCAG 2.5.3 if context is inferred). \n**Action:** When adding confirmation buttons with generic text like 'Yes' or 'No', ALWAYS add descriptive `aria-label` attributes (e.g., `aria-label="Yes, cancel these tickets"`) that include the visible text and clarify the specific action.
