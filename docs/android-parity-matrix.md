...
| Preferences and themes | Persist four themes plus navigation/Ledger preferences. | `theme.ts`, `Profile.tsx`, `saveLedgerLayout()` | `user_prefs`; local DataStore policy. | Profile/settings | Theme token screenshots; account-switch clearing test. | Theme and Ledger-layout persistence both implemented (separate local DataStore instances, no `user_prefs` sync yet); navigation preference persistence and account-switch clearing pending |
