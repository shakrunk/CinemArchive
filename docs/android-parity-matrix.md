...
| Ledger | Present all existing calculations and widget preferences with accessible alternatives. | `ledgerDerive.ts`, `ledgerStats.ts`, `ledgerPanels.ts` | Stable library/history inputs; `user_prefs.ledger_layout`. | Ledger | Shared calculation fixtures; widget persistence; chart semantics. | Contract documented ([ledger.md](./android-contracts/ledger.md)); 5 of 20 widgets implemented on a fixed-order board (hero ribbon + Feature Lengths/On the Air/By the Era/Coming Attractions); 15 remain, several blocked on data not mirrored locally (cast/crew/imdbRating/originalLanguage/companions); the customizable board, layout sync, and accessible alternatives (§5 — the web app's own gap must not be ported as-is) remain |
