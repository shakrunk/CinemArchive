...
## 2026-09-02 - [O(N*M) derived calculation optimization]
**Learning:** When derived properties (like `titleLastInteractionAt`) iterate over deep arrays (e.g. viewings, seasons, episodes, watch events) and are used inside sorting or filtering logic across a large array of elements, this results in redundant O(N*M) computations on every state change because the calculation is executed multiple times for the same unchanged object.
**Action:** Use a `WeakMap` keyed by the object reference (e.g. `WeakMap<Title, number>`) to cache the result of the expensive derived calculation outside of React components. Since the items are immutable in state stores like Zustand, the cached value will accurately reflect the object's reference and be garbage collected when the object is replaced or removed.
