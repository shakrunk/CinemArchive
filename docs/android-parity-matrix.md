...
| Ledger | Present all existing calculations and widget preferences with accessible alternatives. | `ledgerDerive.ts`, `ledgerStats.ts`, `ledgerPanels.ts` | Stable library/history inputs; `user_prefs.ledger_layout`. | Ledger | Shared calculation fixtures; widget persistence; chart semantics. | Contract documented ([ledger.md](./android-contracts/ledger.md)); all 20 widgets implemented (hero ribbon + full board), each with a genuine accessible list alternative per §5 — not the web app's tooltip-only fallback; board is customizable (add/remove/move/resize/settings) via a local edit mode, persisted on-device. Remaining: syncing the layout to `user_prefs.ledger_layout` (blocked on a real `RemoteMutationWriter`, itself blocked on a physical device — see `docs/android-implementation-status.md`), a responsive multi-column grid for the `width` setting (Android is phone-first today, every width renders full-bleed), and wiring `timeRange`/`scope` settings into widget computation (only `topN`/`title` are applied today) |
